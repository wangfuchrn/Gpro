<template slot-scope="scope">
                     ...
                     <el-form-item label="参考答案 " prop="answer">
                        <el-radio-group v-model="form.answer">
                          ...
                          <el-radio :label="'错误'">错误</el-radio>
                              <!-- <el-radio
                                v-for="(option) in judgeAnswerOptions"
                                :key="option.optionName"
                                :label="option.optionName"
                              >{{option.optionName}}</el-radio> -->
                      </el-radio-group>
                    </el-form-item>
                 </el-col>
                 <el-col :span="24">
    ...
    <el-dialog title="预览题目" :visible.sync="dialogViewVisible" width="60%" top="10vh">
        ...
        <ul v-if="form.type === 2" class="subject-options"  style="padding:10px;">
...
<script>
  import {
    ...
    import { judgementlistExamSubjects, removeExamSubjectById } from  "@/api/assess/examManage";
    ...
    export default {
        ...
        data() {
            return {
                // 用户表格数据
                subjectList:this.judgeList,
                // 总条数
                total:this.isJudgeTotal,
                // 遮罩层
                loading: true,
                // 选中数组
                ids: [],
                ...
                // judgeAnswerOptions: [
                //     {optionName: '正确', optionContent: '正确' },
                //     {optionName: '错误', optionContent: '错误' }
                // ],
            }
        ...
        methods: {
            ...
            getList() {
                ...
                judgementlistExamSubjects(examid).then(response => {
                    this.subjectList = response.rows;
                    this.$emit('judgeListEditData',response.rows);
            ...
            // 加载题目信息
              judgegetSubject(row.id).then(response => {
                ...
                this.$refs["form"].validate(valid => {
                    if (valid) {
                        if (this.form.id != undefined) {
                            judgeputSubject(this.form).then(response => {
                            ...
                            judgeaddSubject(this.form).then(response => {
                                if (response.code === 200) {
